...
## Handover / COBie (⚠️ NEPOTVRDENÉ — celá sekcia)

| EN | SK | CZ | Poznámka |
|---|---|---|---|
| COBie | COBie | COBie | vlastný názov/akronym, bez prekladu |
| Handover | Odovzdanie (stavby) | Předání (stavby) | ⚠️ NEPOTVRDENÉ — návrh |
| Asset Operations Handover (AOH) | Asset Operations Handover | Asset Operations Handover | ⚠️ NEPOTVRDENÉ — návrh ponechať bez prekladu, je to názov bSI frameworku/projektu |
| Information Delivery Manual (IDM) | Manuál dodania informácií | Manuál dodání informací | ⚠️ NEPOTVRDENÉ — návrh |
| Maintainable asset | Udržiavateľné aktívum | Udržovatelné aktivum | ⚠️ NEPOTVRDENÉ — návrh; COBie/FM kontext |

## Požiadavky / Compliance (⚠️ NEPOTVRDENÉ — celá sekcia)

| EN | SK | CZ | Poznámka |
|---|---|---|---|
| `IfcConstraint` | Obmedzenie | Omezení | abstraktný supertyp; `ConstraintSource` je voľný text, nie odkaz na dokument |
| `IfcObjective` | Cieľ obmedzenia | Cíl omezení | kvalitatívna stránka — účel/dôvod (napr. súlad s predpisom) |
| `IfcMetric` | Metrika | Metrika | kvantitatívna stránka — konkrétna nameraná/požadovaná hodnota |
| Constraint Grade | Stupeň záväznosti | Stupeň závaznosti | HARD/SOFT/ADVISORY |
| Code Compliance | Súlad s predpisom | Soulad s předpisem | hodnota `IfcObjectiveEnum.CODECOMPLIANCE` |
| `IfcClassification` | Klasifikačný systém | Klasifikační systém | napr. SNIM, CCI — `Location`/`Edition` voliteľné |
| `IfcClassificationReference` | Klasifikačná referencia | Klasifikační reference | konkrétny kód/kľúč v rámci systému |
| `IfcDocumentReference` | Odkaz na dokument | Odkaz na dokument | viaže sa na konkrétnu verziu/URI, nevhodné pre meniacu sa legislatívu |
